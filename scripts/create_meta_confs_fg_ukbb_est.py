...
def generate_json(combined_results):
    ...
    est_conf = {
        'name': 'EST',
        'file': combined_results['est_link'],
        'n_cases': int(combined_results['cases_gwas']),
        'n_controls': int(combined_results['controls_gwas']),
        'chr': 'anew_chr',
        'pos': 'anew_pos',
        'ref': 'REF',
        'alt': 'ALT',
        ...
        'pval': 'p.value',
        'se': 'SE',
        'extra_cols': ["AF.Cases","AF.Controls"]
    }
...
def run():
    ...
    parser.add_argument('in_study_est_file', action='store', type=str, help='A tab-delimited text file with EST phenotype N\'s')
    ...
    # Read est study metadata and drop phenotypes not in mapping
    est = pd.read_csv(args.in_study_est_file, sep='\t')
    est = est[est['Endpoint name'].isin(mapping['est_phenotype'])]
    est.index = pd.Index(est['Endpoint name'], name='est_phenotype')

    # Join study metadatas with mapping
    all = mapping.join(fg, how='inner').join(ukbb, how='inner', rsuffix='_ukbb').join(est, how='inner', rsuffix='_est')
    ...
    all[['fg_link', 'ukbb_link', 'est_link']].to_csv('sumstat_files.txt', header=False, index=False, sep='\t')
    ...
    all['est_link'] = all['est_link'].replace('^gs:/', '/cromwell_root', regex=True)
